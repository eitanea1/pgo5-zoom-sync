...
function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  // Standalone = page is NOT inside an iframe (no Claude Design host). In that
  // case we surface a floating gear button so the panel is reachable without
  // the host's edit-mode toolbar.
  const standalone = React.useMemo(() => {
    try { return window.self === window.top; } catch (e) { return false; }
  }, []);

  // Keyboard shortcut: press "T" anywhere (outside an input) to toggle.
  React.useEffect(() => {
    ...
    return (
      <button
        ...
        style={{
          ...
          boxShadow: '0 8px 24px rgba(0,0,0,0.35), 0 0 0 1px rgba(255,255,255,0.10) inset',
